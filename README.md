...
- 📶 **自定义 WiFi 图标**：信号格样式替换顶栏 WiFi 图标，可选开关、切换即时生效
...
| 特性 | 说明 |
|---|---|
| 📶 **信号格样式** | 3 根柱子外框始终存在（未填充的格子镂空），用蓝色 `#38BDF8` 填充数量表达信号强度 |
| 🔄 **实时映射** | 官方信号强度自动映射：`excellent/good → 3 格`、`ok → 2 格`、`weak/none → 1 格` |
| ⚡ **一键开关** | 设置面板内开关，切换即时生效，关闭后还原系统原 WiFi 图标 |
...
- **WiFi 图标**：启用 / 停用自定义 WiFi 信号格图标，切换即时生效
- **界面语言**：跟随系统 / 中文 / 英文
